...
const Dialog: React.FC<DialogProps> = (props) => {
  ...
  return (
    <RadixDialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <RadixDialog.Portal>
        <RadixDialog.Overlay className="fixed inset-0 z-100 bg-n-900/40 backdrop-blur-[1.5px] data-[state=open]:animate-overlay-show data-[state=closed]:animate-overlay-hide" />
        <RadixDialog.Content
          className={cn(
