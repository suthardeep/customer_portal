...
} from "@/features/reviews/reviewsMutations";
...
interface MyOrderDetailsHeaderProps {
  order: OrderItemDetail;
  enableRating?: boolean;
}

export function MyOrderHeaderDetails({
  order,
  enableRating = false,
}: MyOrderDetailsHeaderProps) {
  const reviewSheet = useToggle();
  const [triggerRating, setTriggerRating] = useState(0);
  const createReviewMutation = useCreateReviewMutation();
  const updateReviewMutation = useUpdateReviewMutation();
  const { data: myReviewsData, isLoading: isReviewLoading } = useQuery(
    reviewQueries.myReviews(),
  );
  ...
  return (
    <>
      <Link
        ...
        search={{ variantId: order.variantId }}
                ...
                <>
                  ...
                  <div className="flex gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <div key={i} className="shimmer size-5 rounded" />
                    ))}
                  </div>
                </>
              ) : (
        ...
        defaultValues={
          existingReview
            ? {
                ...
                mediaUrls: existingReview.mediaUrls,
              }
            : undefined
        }
        onSubmit={(data) => {
