...
const AddressSelector = ({ selectedAddress, onChange }: AddressSelectorProps) => {
  if (!selectedAddress) {
    return (
      <div className="flex items-center justify-between rounded-xl border border-n-400 bg-n-50 p-4">
        <p className="text-sm text-n-700">No address selected</p>
        <Button size="sm" variant="ghost" color="primary" onClick={onChange}>
          Select Address
        </Button>
      </div>
    );
    ...
    <div className="flex items-center gap-3 rounded-xl border border-n-400 bg-n-50 p-4">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {selectedAddress.isDefault && <Chip>Default</Chip>}
        </div>
      ...
      <Button size="sm" variant="ghost" color="neutral" onClick={onChange}>
        Change
      </Button>
    </div>
  );
