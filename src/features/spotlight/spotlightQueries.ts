...
import { getSpotlightProfile, getFeedExplore, getPostById, getCreatorAnalytics, getBookmarkedPosts, getCreatorTiers, getMyPosts } from "./spotlightService";
...
export const spotlightQueries = {
  ...
  bookmarkedPosts: () =>
    infiniteQueryOptions({
      ...
      queryFn: async ({ pageParam }) => {
        const response = await getBookmarkedPosts({ data: { page: pageParam } });
        return response.data;
      },
      initialPageParam: 1,
      getNextPageParam: (lastPage) =>
