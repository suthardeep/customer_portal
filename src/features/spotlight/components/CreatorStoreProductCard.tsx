...
export function CreatorStoreProductCard({
  ...
  return (
    <Link
      ...
      search={{ variantId: product.variantId, affiliateCode: product.affiliateCode }}
    >
      <div className="flex flex-col overflow-hidden rounded-lg border border-n-300 bg-white">
        {/* Image */}
        <div className="relative aspect-square overflow-hidden">
          <Image src={imageUrl ?? ""} alt={product.name} className="size-full object-cover" />
          ...
          <div className="absolute bottom-0 right-0" onClick={(e) => e.preventDefault()}>
        ...
        <div className="flex flex-col gap-1 px-3 pt-1.5 pb-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <p className="font-semibold text-n-900">{formatCurrency(product.price)}</p>
            {product.mrp > product.price && (
              <p className="text-n-600 line-through">{formatCurrency(product.mrp)}</p>
            )}
          </div>
        </div>
      </div>
